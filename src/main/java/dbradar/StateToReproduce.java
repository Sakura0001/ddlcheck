...
public class StateToReproduce {
    ...
    private List<Query> dmlStmts = new ArrayList<>();
     ...
     *
     * @param queryString the query string to be logged
     */
    ...
    public void logDMLStmt(Query query) {
        if (query == null) {
            throw new IllegalArgumentException();
        }
        dmlStmts.add(query);
    ...
    public List<Query> getDMLStmts() {
        return Collections.unmodifiableList(dmlStmts);
    }
    ...
    /**
     * Returns a local state in which a test oracle can save useful information
     * about a single run. If the local state is closed without indicating access to
     * it, the local statements will be added to the global state.
     *
    ...
    /**
     * State information that is logged if the test oracle finds a bug or if an
     * exception is thrown.
     */
