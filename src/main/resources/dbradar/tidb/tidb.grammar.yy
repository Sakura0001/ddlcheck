...
create_table:
    ...
    # | CREATE opt_temporary? TABLE if_not_exist? _new_table_name (first_new_column more_new_column*) table_option*
    # | CREATE opt_temporary? TABLE if_not_exist? _new_table_name LIKE _table  # copy create table
    # | CREATE opt_temporary? TABLE if_not_exist? _new_table_name AS? select # query create table
...
column_constraint: # not complete
    PRIMARY KEY
    ...
    | NULL
    | AUTO_INCREMENT
    @disable-query {((?<!((FLOAT|INT|DOUBLE)( (UN)?SIGNED)?( ZEROFILL)?))|(AUTO_INCREMENT.*)|(SERIAL.*)) AUTO_INCREMENT}
    | SERIAL DEFAULT VALUE
    @disable-query {((?<!((FLOAT|INT|DOUBLE)( (UN)?SIGNED)?( ZEROFILL)?))|(AUTO_INCREMENT.*)|(SERIAL.*)) SERIAL}
...
table_constraint:
    ...
    | , FOREIGN KEY _new_index_name? (_fk_column) foreign_key_clause
    | , key_or_index _new_index_name? (index_table_column) visible_or_invisible?
...
foreign_key_clause:
    REFERENCES _reference_table reference_delete_action? reference_update_action
    @disable-query{(c[0-9]+) [A-Z]*(BLOB|TEXT).*FOREIGN KEY( i[0-9]+)? \(\1}

reference_delete_action:
    ON DELETE reference_control_type

reference_update_action:
...
# CREATE INDEX
create_index:
    CREATE index_key_type? INDEX if_not_exist? _new_index_name index_type? ON _table(index_table_column) index_option?
...
visible_or_invisible:
    VISIBLE
    | INVISIBLE @disable-query {ALTER.*PRIMARY KEY} # A primary key index cannot be invisible

# DROP TABLE
drop_table:
    DROP TABLE if_exists? _table restrict_or_cascade?
    | DROP TABLE if_exists? _table
    | DROP opt_temporary TABLE if_exists? _temp_table restrict_or_cascade?
...
# CREATE VIEW
create_view:
    CREATE VIEW _new_view_name AS simple_select                 @disable-oracle write_guard, write_guard_opt, transaction_verifier
    | CREATE OR REPLACE VIEW _new_view_name AS simple_select    @disable-oracle write_guard, write_guard_opt, transaction_verifier

# DROP VIEW
drop_view:
    DROP VIEW if_exists? _view restrict_or_cascade?
...
join_clause:
    ...
    | join_table NATURAL JOIN join_table
    | join_table join_type join_table
    | join_table LEFT JOIN join_table on_or_use_clause
...
# UPDATE
update:
    UPDATE priority_option? IGNORE? _table SET assignment assignment_more* where_clause?
    | UPDATE priority_option? IGNORE? _table SET assignment assignment_more* where_clause
    @enable-oracle transaction_verifier
    | UPDATE priority_option? IGNORE? _table SET assignment assignment_more* where_clause
    @enable-oracle transaction_verifier
...
# DELETE
delete:
    DELETE priority_option? QUICK? IGNORE? FROM _table where_clause?
    | DELETE priority_option? QUICK? IGNORE? FROM _table where_clause
    @enable-oracle transaction_verifier
    | DELETE priority_option?? QUICK? IGNORE? FROM _table where_clause
    @enable-oracle transaction_verifier
...
# INSERT
insert:
    INSERT priority_option? IGNORE? INTO? _table (_insert_columns) VALUES (_insert_values)
...
# REPLACE
replace:
    REPLACE priority_option? INTO? _table (_insert_columns) VALUES (_insert_values)
...
# DROP INDEX
drop_index:
    DROP INDEX if_exists? _drop_index algorithm_option? lock_option?
...
predicate:
    ...
    | (predicate MEMBER OF ( predicate ))    @disable-oracle equation
    | (expression_atom)
...
str_func:
    ...
    | CONCAT( arg )
    | CONCAT( arg_list )
    | ELT( arg_list )    @disable-oracle equation
...
math_func:
    ...
    | CEIL( arg )
    | CEILING( arg )
    | CONV( arg, _int4_unsigned, _int4_unsigned )    @disable-oracle equation
    | COS( arg )
    | COT( arg )
    | CRC32( arg )
    | DEGREES( arg )
...
# ANALYZE
analyze_table:
    ANALYZE TABLE _table INDEX analyze_option_list?

analyze_option_list:
     WITH analyze_option analyze_option_more*

analyze_option_more:
    , analyze_option

analyze_option:
    _int_1_100000 BUCKETS
    | _int_1_100000 TOPN
    | _int_1_100000 CMSKETCH DEPTH
    | _int_1_100000 CMSKETCH WIDTH
    | analyze_sample_rate

analyze_sample_rate:
    @disable-query {^(?!.*SAMPLERATE.*SAMPLES|.*SAMPLES.*SAMPLERATE).*$} # Cannot set SAMPLERATE and SAMPLES at the same time
    _int_1_100000 SAMPLES
    | _float_0_1 SAMPLERATE
...
# CHECKSUM
checksum:
    ADMIN CHECKSUM TABLE _table more_table*
...
# CHECK TABLE
check_table:
    ADMIN CHECK TABLE _table more_table*

# BEGIN TRANSACTION
begin:
    BEGIN                                          @disable-oracle write_guard_opt
    | BEGIN PESSIMISTIC                            @disable-oracle write_guard_opt
    | BEGIN OPTIMISTIC                             @disable-oracle write_guard
    | START TRANSACTION                            @disable-oracle write_guard_opt
    | START TRANSACTION transaction_characteristic @disable-oracle write_guard, write_guard_opt, equation, transaction_verifier
    # | SET AUTOCOMMIT = 0
...
transaction_characteristic:
    ...
    | READ WRITE
    | READ ONLY # still allows write operations

# COMMIT
commit:
    COMMIT
    | COMMIT completion_type_within_transaction   @disable-oracle write_guard, write_guard_opt, transaction_verifier

# ROLLBACK
rollback:
    ROLLBACK
    | ROLLBACK completion_type_within_transaction @disable-oracle write_guard, write_guard_opt, transaction_verifier

completion_type_within_transaction:
...
# ISOLATION LEVEL
set_isolation:
    SET global_or_session? TRANSACTION transaction_chars
...
set_variable:
    ...
    | SET global_or_session cte_max_recursion_depth = _int16_unsigned
    ...
    | SET global_or_session tidb_opt_agg_push_down = on_or_off
    | SET global_or_session tidb_build_stats_concurrency = _int8_unsigned
    | SET SESSION tidb_checksum_table_concurrency = _int8_unsigned
    | SET global_or_session tidb_distsql_scan_concurrency = _int8_unsigned
    | SET global_or_session tidb_index_lookup_size = _int24_unsigned
    | SET global_or_session tidb_executor_concurrency = _int8_unsigned
    | SET global_or_session tidb_index_serial_scan_concurrency = _int8_unsigned
    | SET global_or_session tidb_index_join_batch_size = _int24_unsigned
    | SET global_or_session tidb_skip_utf8_check = on_or_off
    | SET global_or_session tidb_opt_insubq_to_join_and_agg = on_or_off
    | SET global_or_session tidb_opt_correlation_exp_factor = _int24_unsigned
